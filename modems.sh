...
iptables -A FORWARD_MODEM -o "$ext1" -d "$ip_modem1" -j ACCEPT
iptables -A FORWARD_MODEM -o "$ext2" -d "$ip_modem2" -j ACCEPT
iptables -A FORWARD_MODEM -o "$ext3" -d "$ip_modem3" -j ACCEPT
...
# вывод
echo
output "`iptables-save | grep MODEM`"

#####################################
### SQUID ###
# настройки по модемам внечены непосредственно в сами конф файлы squid (tcp_outgoing_address.conf)
